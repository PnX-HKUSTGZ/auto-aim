...
public:
    RuneBallistic(
        double k = 0.1, double K = 0.3, double bulletV = 30,
...
#define SMALL_RUNE_CURVE(x, a, b, c, sign) (((a) * ((x) + (b)) + (c)) * (sign))
    ...
    struct CostFunctor
    {
        ...
        double height;
        double theta;
        explicit CostFunctor(
            RuneBallistic & ballistic, double & distance, double & height, double & theta)
        : ballistic_ref(ballistic), distance(distance), height(height), theta(theta)
        {
        }
        ...
        bool operator()(const T * const t, T * residual) const
        {
            ...
            residual[0] =
                T(1 / ballistic_ref.k) *
                    ceres::log(T(ballistic_ref.k) * ceres::cos(theta) * v0 * (*t) + T(1.0)) -
                T(ceres::sqrt(distance_aim * distance_aim + height_aim * height_aim));
            return true;
        }
    };
    enum class EulerOrder { XYZ, XZY, YXZ, YZX, ZXY, ZYX };
    Eigen::Matrix3d eulerToMatrix(
        const Eigen::Vector3d & euler, EulerOrder order = EulerOrder::XYZ) const
    {
