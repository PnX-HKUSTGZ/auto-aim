...
RuneBallistic::RuneBallistic(
    double k, double K, double bulletV, Eigen::Vector3d odom2gunxyz, Eigen::Vector3d odom2gunrpy)
{
...
std::pair<double, double> RuneBallistic::iteration(
    double & thres, double & init_pitch, double & initT)
{
    ...
    Eigen::Vector3d last_target =
        getTarget(update_tmp_theta_t.second + target_msg.header.stamp.sec);
...
double RuneBallistic::predictAngle(double time)
{
    ...
    if (target_msg.is_big) {
        pred_angle = BIG_RUNE_CURVE(
            current_time, target_msg.fitting_curve[0], target_msg.fitting_curve[1],
            target_msg.fitting_curve[2], target_msg.fitting_curve[3], target_msg.fitting_curve[4],
...
double RuneBallistic::optimizeTime(double initial_guess, double & l, double & h, double & theta)
{
    ...
    problem.AddResidualBlock(
        new ceres::AutoDiffCostFunction<RuneBallistic::CostFunctor, 1, 1>(
            new RuneBallistic::CostFunctor(*this, l, h, theta)),
        nullptr, &t);
