...
void ArmorTrackerNode::initializeEKF()
{
  ...
  p0.setIdentity();
  tracker_->ekf =
    ExtendedKalmanFilter{f, h1, h2, h_two, j_f, j_h1, j_h2, j_h_two, u_q, u_r, u_r_two, p0};
}
...
void ArmorTrackerNode::publishImg(
  const auto_aim_interfaces::msg::Target & target_msg, const sensor_msgs::msg::Image & image_msg)
{
  ...
  if (target_msg.tracking) {
    ...
    for (size_t i = 0; i < a_n; i++) {
      ...
      for (size_t j = 0; j < corners_image.size(); ++j) {
        cv::line(
          image, corners_image[j], corners_image[(j + 1) % corners_image.size()],
          cv::Scalar(0, 255, 0), 2);
      }
    }
  }
  ...
  cv::putText(
    image, text.str(), cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 255, 0), 2);
