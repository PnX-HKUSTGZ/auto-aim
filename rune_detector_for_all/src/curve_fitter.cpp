...
CurveFitter::CurveFitter(const MotionType & t) : type_(t), fitting_future_(nullptr)
{
...
void CurveFitter::fitDoubleCurve()
{
    ...
    std::for_each(data_history_queue_.begin(), data_history_queue_.end(), [&](const auto & data) {
        small_fitting_problem.AddResidualBlock(
            ...
            new ceres::CauchyLoss(0.5), small_param_ptr);
        ...
        big_fitting_problem.AddResidualBlock(
            ...
            new ceres::CauchyLoss(0.5), big_param_ptr);
    });
    ...
    RCLCPP_DEBUG(
        rclcpp::get_logger("curve_fitter"), "Fitting time: %ld ms",
...
std::string CurveFitter::getDebugText()
{
    ...
    if (type_ == MotionType::BIG) {
        ...
        t = fmt::format(
            "V: {}( {:.2f} sin( {:.2f} (x {} {:.2f}) ) {} {:.2f} )",
            direction_ == Direction::CLOCKWISE ? "-" : " ", a, omega, (d > 0 ? "+" : "-"),
            std::abs(d), (b > 0 ? "+" : "-"), std::abs(b));
...
void CurveFitter::update(double time, double angle)
{
    ...
    direction_ =
        static_cast<int>(angle_diff < 0 ? Direction::CLOCKWISE : Direction::ANTI_CLOCKWISE);
    ...
    if (fitting_future_ == nullptr) {
        ...
        RCLCPP_DEBUG(
            rclcpp::get_logger("curve_fitter"),
            "Fitting is in progress, do not start a new fitting");
    }
}
...
{
    return !(
        type_ == MotionType::UNKNOWN || direction_ == Direction::UNKNOWN ||
