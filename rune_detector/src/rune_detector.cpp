...
std::vector<RuneObject> RuneDetector::detectRune(const cv::Mat & img, int min_lightness)
{
    ...
    if (signal_points_hitting.size() != 6)
        return {};
    else {
    ...
    std::sort(
        signal_points_hit.begin(), signal_points_hit.end(),
        [this](const std::vector<cv::Point2f> & a, const std::vector<cv::Point2f> & b) {
...
std::vector<cv::Point2f> RuneDetector::processHittingLights()  // 用于绘制的原图或当前帧
{
    ...
    for (const auto & contour : contours) {
        ...
        double aspect_ratio =
            static_cast<double>(rotated_rect.size.width) / rotated_rect.size.height;
        if (aspect_ratio < 1.0) {
    ...
    if (matched) {
        ...
        for (int i = 0; i < 4; i++) {
            cv::line(
                frame, flow_light_vertices[i], flow_light_vertices[(i + 1) % 4],
                cv::Scalar(0, 255, 0), 2);
        }
        // 绘制匹配的椭圆
        ...
        for (auto & flow_light_vertice : flow_light_vertices) {
            int_flow_light_vertices.push_back(cv::Point(
                static_cast<int>(flow_light_vertice.x), static_cast<int>(flow_light_vertice.y)));
        }
...
std::vector<std::vector<cv::Point2f>> RuneDetector::processhitLights()
{
    ...
    for (const auto & matched_arm_light : matched_arm_lights) {
        ...
        for (int i = 0; i < 4; i++) {
            cv::line(
                frame, arm_light_vertices[i], arm_light_vertices[(i + 1) % 4],
                cv::Scalar(0, 0, 255), 2);
        }
        // 绘制匹配的椭圆
...
std::tuple<cv::Point2f, cv::Mat> RuneDetector::detectRTag(
    const cv::Mat & img, const cv::Point2f & prior)
{
...
// 计算角度差
double RuneDetector::calculateAngleDifference(
    const cv::RotatedRect & rect, const cv::RotatedRect & ellipse)
{
...
// 计算椭圆沿特定方向的轴的长度
double RuneDetector::calculateAxisLength(
    const cv::RotatedRect & ellipse, const cv::Point2f & direction)
{
    // 椭圆的长轴和短轴长度
    ...
    // 计算沿特定方向的弦的长度
    double chord_length =
        2 * a * b / std::sqrt(b * b * x_prime * x_prime + a * a * y_prime * y_prime);

    return chord_length;
}
// 计算比例差
double RuneDetector::calculateRatioDifferenceHitting(
    const cv::RotatedRect & rect, const cv::RotatedRect & ellipse)
{
...
// 计算匹配程度
double RuneDetector::calculateMatchScoreHitting(
    const cv::RotatedRect & rect, const cv::RotatedRect & ellipse)
{
    ...
    if (angle_diff < CV_PI / 12 && ratio_score < 0.2)
        return angle_score + ratio_score;
    else
        return -1;  // 不匹配
}
// 计算比例差
double RuneDetector::calculateRatioDifferencehit(
    const cv::RotatedRect & rect, const cv::RotatedRect & ellipse)
{
...
// 计算匹配程度
double RuneDetector::calculateMatchScorehit(
    const cv::RotatedRect & rect, const cv::RotatedRect & ellipse)
{
    double angle_diff = calculateAngleDifference(rect, ellipse);
...
// 计算指定方向与椭圆的两个交点
std::vector<cv::Point2f> RuneDetector::ellipseIntersections(
    const cv::RotatedRect & ellipse, const cv::Point2f & dir)
{
    // 单位化方向
...
std::vector<cv::Point2f> RuneDetector::getSignalPoints(
    const cv::RotatedRect & ellipse, const cv::RotatedRect & rect)
{
    // 最终返回的 6 个点
    ...
    std::sort(
        four_pts.begin(), four_pts.end(), [base](const cv::Point2f & p1, const cv::Point2f & p2) {
...
cv::Rect RuneDetector::calculateROI(const cv::RotatedRect & rect)
{
    ...
    // 计算 ROI 并检查边界
    cv::Rect roi(
        b.x - roi_side_length / 2, b.y - roi_side_length / 2, roi_side_length, roi_side_length);
...
// 计算点到椭圆的距离
double RuneDetector::pointToEllipseDistance(
    const cv::Point2f & point, const cv::RotatedRect & ellipse)
{
    // 将点从图像坐标系转换到椭圆坐标系
...
cv::RotatedRect RuneDetector::detectBestEllipse(const cv::Mat & src)
{
    ...
    for (int i = 0; i < STANDARD_SIZE; i++) {
        for (int j = 0; j < STANDARD_SIZE; j++) {
            if (resized.at<uchar>(i, j) > 0 &&
                cv::norm(cv::Point(j, i) - cv::Point(STANDARD_SIZE / 2, STANDARD_SIZE / 2)) * 2 <
                    STANDARD_SIZE) {
    ...
    if (standard_ellipse.size.width > 0 && standard_ellipse.size.height > 0) {
        best_ellipse = cv::RotatedRect(
            ...
            cv::Size2f(
                standard_ellipse.size.width / scale_x, standard_ellipse.size.height / scale_y),
...
bool RuneDetector::isRightColor(const cv::RotatedRect & rect)
{
    // 计算 ROI
    cv::Rect roi_rect(
        rect.center.x - rect.size.width / 2, rect.center.y - rect.size.height / 2, rect.size.width,
        rect.size.height);
