...
: Node("ballistic_calculation", options)
{
    ...
    std::vector<double> xyz_vec =
        this->declare_parameter("xyz", std::vector<double>{0.0, 0.0, 0.0});
    ...
    subscription_ = this->create_subscription<auto_aim_interfaces::msg::Target>(
        ...
        std::bind(&BallisticCalculateNode::targetCallback, this, std::placeholders::_1));
    //创建发布者
    publisher_ = this->create_publisher<auto_aim_interfaces::msg::Firecontrol>("/firecontrol", 10);
    //设置时间callback
    last_fire_time = this->now();
    timer_ = this->create_wall_timer(
        std::chrono::milliseconds(1), std::bind(&BallisticCalculateNode::timerCallback, this));
...
void BallisticCalculateNode::timerCallback()
{
    ...
    //进入第一次大迭代
    double init_pitch = std::atan(
        target_msg->position.z / std::sqrt(
                                     target_msg->position.x * target_msg->position.x +
                                     target_msg->position.y * target_msg->position.y));
    double init_t = std::sqrt(
                        target_msg->position.x * target_msg->position.x +
                        target_msg->position.y * target_msg->position.y) /
                    (cos(init_pitch) * this->calculator->bulletV);

    std::pair<double, double> first_iteration_result =
        this->calculator->iteration1(THRES1, init_pitch, init_t);
    ...
    if (target_msg->armors_num == 4 or target_msg->armors_num == 3) {
        std::vector<double> hit_aim =
            calculator->predictInfantryBestArmor(temp_t, min_v, max_v, v_yaw_PTZ);
        ...
        if (hit_aim.size() == 4) {  //如果返回的结果是4个，说明是第三种策略
            //计算是否开火
            iffire_result = calculator->iteration2(
                THRES2, temp_theta, temp_t, hit_aim_fire[0], hit_aim_fire[1], hit_aim_fire[2]);
            //计算瞄准目标
            final_result =
                calculator->iteration2(THRES2, temp_theta, temp_t, chosen_yaw, hit_aim_fire[1], r);
        } else {
            //进入第二次大迭代
    ...
    RCLCPP_DEBUG(
        this->get_logger(), "Projected point: (%.2f, %.2f), Normalized: (%.2f, %.2f)",
