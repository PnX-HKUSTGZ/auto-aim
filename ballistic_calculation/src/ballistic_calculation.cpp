...
std::pair<double, double> Ballistic::iteration2(
  double & thres, double & init_pitch, double & initT, double & yaw, double & z, double & r)
{
  theta = init_pitch;
  double differ;
  double t;
  std::pair<double, double> updateTmpThetaT;
  double x =
    target_msg.position.x + target_msg.velocity.x * initT + r * cos(yaw + target_msg.v_yaw * initT);
  double y =
    target_msg.position.y + target_msg.velocity.y * initT + r * sin(yaw + target_msg.v_yaw * initT);
  ...
  double fx =
    target_msg.position.x + target_msg.velocity.x * updateTmpThetaT.second + r * cos(newyaw);
  double fy =
    target_msg.position.y + target_msg.velocity.y * updateTmpThetaT.second + r * sin(newyaw);
...
double Ballistic::optimizeTime1(double initial_guess)
{
  ...
  problem.AddResidualBlock(
    new ceres::AutoDiffCostFunction<Ballistic::CostFunctor1, 1, 1>(
      new Ballistic::CostFunctor1(*this)),
    nullptr, &t);
...
double Ballistic::optimizeTime2(double initial_guess, double & x, double & y)
{
  ...
  ceres::Problem problem;
  problem.AddResidualBlock(
    new ceres::AutoDiffCostFunction<Ballistic::CostFunctor2, 1, 1>(
      new Ballistic::CostFunctor2(*this, x, y)),
    nullptr, &t);
...
三级策略：高速瞄中心
*/
std::vector<double> Ballistic::predictInfantryBestArmor(
  double T, double min_v, double max_v, double v_yaw_PTZ)
{
...
std::vector<double> Ballistic::stategy_1(double T)
{
  ...
  // 找到最小的角度对应的装甲板
  sort(angles.begin(), angles.end(), [](const double & a, const double & b) {
    return std::abs(a) < std::abs(b);
  });
  armor_info chosen_armor;
  chosen_armor = armorlist_map[angles[0]];
  if (
    abs(angles[1]) - abs(angles[0]) < 15.0 * pi / 180.0 &&
    abs(chosen_armor.yaw - last_yaw) > abs(armorlist_map[angles[1]].yaw - last_yaw)) {
...
std::vector<double> Ballistic::stategy_1_AimingOutpost(double T)  //4 armors -> 3 armors
{
  // C++
  const double pi = 3.1415926;
...
std::vector<double> Ballistic::stategy_2(double T, double v_yaw_PTZ)
{
  ...
  // 找到最小的角度对应的装甲板
  std::sort(angles.begin(), angles.end(), [](const double & a, const double & b) {
    return std::abs(a) < std::abs(b);
  });
  ...
  /*
    ...
    */
  double yaw = findYaw(
    abs(target_msg.v_yaw), v_yaw_PTZ, sqrt(pow(newxc, 2) + pow(newyc, 2)),
    chosen_armor.r);  // 通过迭代计算得到的 yaw
  if (std::isnan(yaw)) {
    return {chosen_armor.yaw - target_msg.v_yaw * T, chosen_armor.z, chosen_armor.r};
  }
  if (abs(angles[0]) > yaw) {
...
double Ballistic::findYaw(double v_yaw, double v_yaw_PTZ, double distance, double radius)
{
  ...
  for (int i = 0; i < max_iter; ++i) {
    ...
    double df =
      (cos(yaw + yaw_PTZ) * (1 + dyaw_PTZ)) / distance - (cos(yaw_PTZ) * dyaw_PTZ / radius);
...
double Ballistic::angleBetweenVectors(
