    ...
    double & thres, double & init_pitch, double & initT, double & yaw, double & z, double & r)
{
    ...
    double t;
    std::pair<double, double> updateTmpThetaT;
    double x = target_msg.position.x + target_msg.velocity.x * initT -
               r * cos(yaw + target_msg.v_yaw * initT);
    double y = target_msg.position.y + target_msg.velocity.y * initT -
               r * sin(yaw + target_msg.v_yaw * initT);
...
std::vector<double> Ballistic::stategy_1(double T)
{
    ...
    if (abs(angles[1]) - abs(angles[0]) < 15.0 * pi / 180.0 &&
