...
public:
  Ballistic(
    double k = 0.1, double K1 = 0.3, double K2 = 0.3, double bulletV = 30,
    Eigen::Vector3d odom2gun = Eigen::Vector3d(0, 0, 0));  //构造函数
  ...
  std::vector<double> stategy_1_AimingOutpost(double T);
  ...
  std::pair<double, double> iteration2(
  ...
  struct CostFunctor1
  {
    ...
    bool operator()(const T * const t, T * residual) const
    {
      ...
      residual[0] =
        T(1 / ballistic_ref.k) *
          ceres::log(T(ballistic_ref.k) * ceres::cos(ballistic_ref.theta) * v0 * (*t) + T(1.0)) -
        T(ceres::sqrt(futurex * futurex + futurey * futurey));
  ...
  struct CostFunctor2
  {
    ...
    explicit CostFunctor2(Ballistic & ballistic, double & x, double & y)
    : ballistic_ref(ballistic), x(x), y(y)
    {
    }
    ...
    bool operator()(const T * const t, T * residual) const
    {
      ...
      residual[0] =
        T(1 / ballistic_ref.k) *
          ceres::log(T(ballistic_ref.k) * ceres::cos(ballistic_ref.theta) * v0 * (*t) + T(1.0)) -
        T(ceres::sqrt(x_aim * x_aim + y_aim * y_aim));
      return true;
    }
  };
