...
public:
        ...
        const std::string & coord_frame_name)
    {
        ...
        if (object_points_map_.find(coord_frame_name) != object_points_map_.end()) {
            ...
            int solutions = cv::solvePnPGeneric(
                object_points, image_points, camera_matrix_, distortion_coefficients_, rvecs, tvecs,
                false, method_);
            return solutions > 0;
        } else {
            return false;
        ...
        const std::string & coord_frame_name)
    {
        if (object_points_map_.find(coord_frame_name) != object_points_map_.end()) {
            const auto & object_points = object_points_map_[coord_frame_name];
            return cv::solvePnP(
                object_points, image_points, camera_matrix_, distortion_coefficients_, rvec, tvec,
