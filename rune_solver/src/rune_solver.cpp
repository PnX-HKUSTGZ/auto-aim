...
Eigen::Matrix4d RuneSolver::solvePose(const auto_aim_interfaces::msg::Rune & predicted_target)
{
    ...
    std::transform(
        predicted_target.pts.begin(), predicted_target.pts.end(), image_points.begin(),
    ...
    if (pnp_solver && pnp_solver->solvePnP(image_points, rvec, tvec, "rune")) {
        ...
        try {
            ...
            try {
                auto transform = tf2_buffer_->lookupTransform("odom", ps.header.frame_id, ps.header.stamp);
                tf2::doTransform(ps, ps, transform);
            } catch (const tf2::ExtrapolationException & ex) {
                auto fallback_transform = tf2_buffer_->lookupTransform("odom", ps.header.frame_id, tf2::TimePointZero);
...
double RuneSolver::getNormalAngle(const auto_aim_interfaces::msg::Rune::SharedPtr received_target)
{
    ...
    std::transform(
        received_target->pts.begin() + 3, received_target->pts.end(), armor_points.begin(),
        [](const auto & pt) { return cv::Point2f(pt.x, pt.y); });
