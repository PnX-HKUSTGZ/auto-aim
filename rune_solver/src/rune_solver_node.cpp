...
RuneSolverNode::RuneSolverNode(const rclcpp::NodeOptions & options) : Node("rune_solver", options)
{
    ...
    if (debug_) {
        ...
        fitter_text_pub_ = this->create_publisher<std_msgs::msg::String>(
            "rune_solver/fitting_info", rclcpp::SensorDataQoS());
        // Marker
    ...
    pub_timer_ = this->create_wall_timer(
        std::chrono::milliseconds(4), std::bind(&RuneSolverNode::timerCallback, this));
}
